...
import { Camera, Trash2, X, Box } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ sim, selectedBodyId, onClose, isMobile }) => {
  ...
  return (
    <motion.aside
      ...
      className={`fixed bg-[#11141b]/95 backdrop-blur-3xl border-white/10 flex flex-col overflow-hidden shadow-2xl z-[120] pointer-events-auto will-change-transform ${isMobile
        ? 'bottom-0 left-0 right-0 w-full rounded-t-3xl border-t'
        : 'top-4 right-4 bottom-20 w-[320px] rounded-2xl border'
        }`}
        ...
        <>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[2px] text-gray-400">Object Properties</span>
            </div>
            {!isMobile && (
              <button
                onClick={onClose}
          ...
          <section className="flex flex-col gap-6 h-full overflow-y-auto p-4">
            <section>
          ...
          <div className="mt-auto p-4 border-t border-white/10 grid grid-cols-2 gap-3">
            <button
              onClick={() => {
              ...
              className={`w-full py-3 flex items-center justify-center rounded-xl text-[12px] font-semibold tracking-wider transition-colors border ${sim.camera.followingId === body.id
            ...
            <button
              ...
              title="Delete Object"
              className="w-full py-3 flex items-center justify-center rounded-xl text-[12px] font-semibold tracking-wider transition-colors border bg-white/5 border-white/10 text-gray-400 hover:bg-red-900/40 hover:text-red-400 hover:border-red-500/50"
            >
              <Trash2 size={20} />
            </button>
            ...
            <button
              ...
              className={`w-full py-3 flex items-center justify-center rounded-xl text-[10px] uppercase font-extrabold tracking-[2px] transition-all border col-span-2 ${sim.isStar(body)
            ...
            >
              {sim.isStar(body) ? 'Initiate Supernova' : 'Destruct Object'}
            </button>
          </div>
        </>
      )}
